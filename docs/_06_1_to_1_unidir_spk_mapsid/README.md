...
<h2>One-to-One unidirectional mapping. Shared primary key with @MapsId</h3>
...
It uses One-to-One unidirectional mapping through shared primary key with `@MapsId`

![](./one-to-one-unidir-spk-mapsid.png)
...
This console already has tables `CONSUMERS`, `DOMICILES`.

In SQL statement section run

```sql
SELECT * FROM CONSUMERS;
SELECT * FROM DOMICILES;
```

Click `Run`. Beneath of the SQL statement section appears
tables `CONSUMERS`, `DOMICILES`.
It's empty for now.
...
Code example can be found [here](../../src/main/java/com/example/Spring_Boot_Jpa_ER/_06_1_to_1_unidir_spk_mapsid).
...
| Method    | URL                      | Action                  |
|-----------|--------------------------|-------------------------|
| GET       | `/api/v1/consumers`      | Get all Consumers       |
| GET       | `/api/v1/consumers/{id}` | Get a Consumer by id    |
| POST      | `/api/v1/consumers`      | Add new Consumer        |
| PUT       | `/api/v1/consumers/{id}` | Update a Consumer by id |
| DELETE    | `/api/v1/consumers/{id}` | Delete a Consumer by id |
...
Get all Consumers
```text
GET http://localhost:8080/api/v1/consumers
```
The REST-request can be run in following ways:
* Data do not exist.
* Data exist.

Get a Consumer by `id` (e.g., `id` is `1`)
```text
GET http://localhost:8080/api/v1/consumers/1
```
The REST-request can be run in following ways:
...
Add new Consumer
```text
POST http://localhost:8080/api/v1/consumers
```
...
Update a Consumer by `id` (e.g., `id` is `2`)
```text
PUT http://localhost:8080/api/v1/consumers/2
```
request body
```json
{
...
Delete a Consumer by `id` (e.g., `id` is `3`)
```text
DELETE http://localhost:8080/api/v1/consumers/3
```
The REST-request can be run in following ways:
* `id` does not exist (no data with such `id`).
* `id` exists.
...
Postman collection can be found [here](./REST_API_One-to-One_UniDir_SPK_MAPSID.postman_collection.json).


### UML

UML class-diagram can be found [here](./1-to-1-unidir-spk-mapsid-uml.puml).
